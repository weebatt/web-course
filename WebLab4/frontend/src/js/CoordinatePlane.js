...
export function redrawCoordinatePlane(radius = 1) {
    ...
    drawRectangle(originX, originY, 2 * unitSegment * (1 / radius), unitSegment * (1 / radius));
    drawEllipse(`M ${originX} ${originY - unitSegment * (1 / (radius * 2) * 2)} A ${unitSegment * (1 / (radius * 2) * 2)} ${unitSegment * (1 / (radius * 2) * 2)} 0 0 1 ${originX + unitSegment * (1 / (radius * 2) * 2)} ${originY} L ${originX} ${originY} Z`);
}