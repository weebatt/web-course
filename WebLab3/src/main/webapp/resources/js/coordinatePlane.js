...
function drawCoordinatePlane() {
    radius = document.querySelector('input[name="form:rNumberSelector"]:checked');
    if (radius === null){
        radius = 1;
    } else {
        radius = radius.value;
    ...
    while (svg.firstChild) {
        ...
        let scale = radius; // Используйте радиус как коэффициент масштабирования
    ...
    for (let i = 50; i < width; i += tickSpacing) {
        ...
        let pointer = ((i - originX) / tickSpacing) * scale;
        if ((i - originX) === 0) {
...
function drawRectangle(x, y, width, height, radius) {
...
function redrawCoordinatePlane() {
    // console.log("redrawCoordinatePlane() was called!")
